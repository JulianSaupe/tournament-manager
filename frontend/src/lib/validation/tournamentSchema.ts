...
export const tournamentRoundSchema = z.object({
	name: z
		...
		.max(255, 'Round name must be at most 255 characters'),
...
export const tournamentSchema = z
	.object({
		name: z
			...
			.max(255, 'Tournament name must be at most 255 characters'),
		description: z
			.string()
			...
			for (const round of data.rounds) {
				if (!data.allowUnderfilledGroups) {
					...
					if (previousRound === null) {
						...
						const advancingFromPrevious =
							previousRound.playerAdvancementCount * previousRound.groupCount;
