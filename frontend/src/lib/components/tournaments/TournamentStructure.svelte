<script lang="ts">
    ...
    $effect(() => {
        if (autoCalculate && rounds.length > 0) {
            ...
            if (updatedRounds[0]) {
                const newGroupCount = Math.ceil(
                    $tournamentForm.playerCount / updatedRounds[0].playersPerGroup
                );
    ...
    <div class="card-body">
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            <label class="label cursor-pointer justify-start">
                ...
                <input bind:checked={autoCalculate} class="toggle toggle-primary" type="checkbox"/>
            </label>
            <div class="text-sm text-base-content/70">
                When enabled, group counts are automatically calculated based on player count and
                advancement
            </div>
        </div>
        ...
        <div class="mb-6 overflow-x-auto">
            <div class="flex min-w-fit items-center gap-3 p-2">
                {#each rounds as round, index}
                    <div class="flex flex-col items-center">
                        ...
                        >
                            ...
                            <div class="mt-1 text-sm">{round.groupCount * round.playersPerGroup} players</div>
                        </div>
                    </div>

                    {#if index < rounds.length - 1}
                        <div class="flex flex-col items-center px-2">
                            <div class="mb-1 text-center text-xs text-base-content/60">
                                {round.advancingPlayersPerGroup * round.groupCount} advance
                            </div>
                            <svg class="h-4 w-6 text-base-content/40" fill="currentColor" viewBox="0 0 20 10">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <h4 class="text-lg font-medium">{round.name}</h4>
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
                            ...
                            <div class="flex flex-col">
                                <label class="mb-1 text-sm font-medium text-base-content/80"
                                       for="playersPerGroup-{index}">Players per Group</label>
                                <div class="join">
                                    <input
                            ...
                            <div>
                                <label class="mb-1 text-sm font-medium text-base-content/80"
                                       for="matchesPerGroup-{index}">Matches per Group</label>
                                <input
                            ...
                            <div>
                                <label class="mb-1 text-sm font-medium text-base-content/80"
                                       for="advancingPlayers-{index}">Advancing per Group</label>
                                {#if isFinalRound(index)}
                                    ...
                                    <input
                                            ...
                                            disabled
                                    />
                                    <div class="mt-1 text-xs text-base-content/60">winner only</div>
                                {:else}
                                    <!-- Regular rounds -->
                                    <input
                                            id="advancingPlayers-{index}"
                            ...
                            <div>
                                <label class="mb-1 text-sm font-medium text-base-content/80"
                                       for="concurrentGroups-{index}">Concurrent Groups</label>
                                {#if round.groupCount === 1}
                                    ...
                                    <input
                                            ...
                                            disabled
                                    />
                                    <div class="mt-1 text-xs text-base-content/60">single group</div>
                                {:else}
                                    <!-- Multiple groups -->
                                    <input
                                            id="concurrentGroups-{index}"
                            ...
                            <div class="rounded bg-base-200 p-3 text-center md:col-span-2 lg:col-span-1">
                                <div class="mb-1 text-sm font-medium text-base-content/80 lg:hidden">Summary</div>
                                <div class="text-sm font-semibold">
                                    {round.groupCount * round.matchesPerGroup} matches
                                </div>
                                {#if !isFinalRound(index)}
                                ...
                                {:else}
                                    <div class="mt-1 text-xs text-base-content/70">1 winner</div>
                                {/if}
                            </div>
                        </div>
                    </div>
        ...
        <div class="mt-6 text-center">
            <button class="btn btn-primary" onclick={() => addRound()} type="button">
