<script lang="ts">
    ...
    import {statusConfig} from '$lib/ui/config/tournament-status';
    import {tournaments as tournamentStore, setTournaments, addTournament} from '$lib/application/tournament-store';
    import {getWebSocketService} from '$lib/adapters/websocket/websocket-service';
    ...
    onMount(() => {
        ...
        ws.connect().then(() => {
            unsubscribe = ws.subscribe('tournament.created', (payload: any) => {
                const parsedStatus: TournamentStatus = payload.status?.toLowerCase() as TournamentStatus ?? TournamentStatus.DRAFT;
    ...
    function getTournamentsByStatus(tournaments: Tournament[]): Record<string, Tournament[]> {
    ...
    const tournaments = $derived($tournamentStore);
    const tournamentsByStatus = $derived(getTournamentsByStatus(tournaments));
    const activeTournaments = $derived(tournamentsByStatus[TournamentStatus.ACTIVE] || []);
    const totalTournaments = $derived(tournaments.length);
    const activeTournamentCount = $derived(activeTournaments.length);
    const draftTournamentCount = $derived((tournamentsByStatus[TournamentStatus.DRAFT] || []).length);
    const completedTournamentCount = $derived((tournamentsByStatus[TournamentStatus.COMPLETED] || []).length);
        ...
        <div class="mb-6 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
            ...
            <div class="flex gap-2">
                ...
                <div class="dropdown dropdown-end">
                    <div class="btn btn-outline btn-sm" role="button" tabindex="0">
                        <Funnel/>
                        <span class="hidden sm:inline">Filter</span>
                    </div>
                    <ul class="dropdown-content menu z-[1] w-52 rounded-box bg-base-100 p-2 shadow">
                        <li>
                            <button class="justify-between" onclick={() => (statusFilter = null)}>
                                All
                        ...
                        <li>
                            <button
                                    ...
                                    onclick={() => (statusFilter = TournamentStatus.ACTIVE)}
                            >
                                Active
                        ...
                        <li>
                            <button
                                    class="justify-between"
                                    onclick={() => (statusFilter = TournamentStatus.DRAFT)}
                            >
                                Draft
                        ...
                        <li>
                            <button
                                    class="justify-between"
                                    onclick={() => (statusFilter = TournamentStatus.COMPLETED)}
                            >
                                Completed
                        ...
                        <li>
                            <button
                                    class="justify-between"
                                    onclick={() => (statusFilter = TournamentStatus.CANCELLED)}
                            >
                                Cancelled
